...
def retrieve_revenue(company):
  ...
  stock = company
  print(stock)
  IS = requests.get(f'https://financialmodelingprep.com/api/v3/financials/income-statement/{company}?apikey=04e03568cc552607dad12834571400da')
  IS = IS.json()
  IS = IS['financials']
  Revenues = []
  Dates = []
...
def retrieve_revenue(company):
  ...
  datapoints = {'data': [go.Bar(x=Dates, y=Revenues, marker_color='lightsalmon', name='Net Income')],'layout':
    dict(xaxis={'title': 'Date'}, yaxis={'title': 'Net Income'},)}
  return datapoints
